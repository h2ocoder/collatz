...
- **Duality entropy convention.** $H_K$ is computed over $K$ Kozyrev shell-energies (the constant mode $c_0$ is separated out by the Haar split and excluded). $H_W$ is computed over $K{+}1$ Walsh weight-energies and *includes* the $w = 0$ bucket (which equals $c_0^2$). For DC-heavy fields (e.g. dropping-set indicators), this asymmetry inflates $H_W$ relative to $H_K$ and biases the duality scatter toward the "Kozyrev-localized" half-plane. Treat the absolute position in $(H_K, H_W)$-space as a convention-dependent diagnostic; *relative* positions across fields are the robust signal.
